...
export function BottomNav() {
  ...
  return <View style={[styles.nav, { width: Math.min(Math.max(width - 40, 0), 440) }, Platform.OS === 'web' && ({ position: 'fixed' } as never)]}>{items.map((item) => { const current = item.id === segment || (item.id === 'index' && isHome); return <Pressable key={item.id} onPress={() => router.replace(item.href as never)} style={styles.navHit}><View style={[styles.navCircle, current && styles.navCurrent]}><Ionicons name={item.icon} size={25} color={current ? colors.ink : '#F2EFF6'} /></View></Pressable>; })}</View>;
}
...
const styles = StyleSheet.create({
  ...
  nav: { height: 62, borderRadius: 31, backgroundColor: colors.navy, position: 'absolute', bottom: 14, alignSelf: 'center', flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', zIndex: 100, elevation: 20 }, navHit: { width: 68, height: 58, alignItems: 'center', justifyContent: 'center' }, navCircle: { height: 50, width: 50, borderRadius: 25, alignItems: 'center', justifyContent: 'center' }, navCurrent: { backgroundColor: '#FFFFFF' },
